 ...
 * $Id: FieldMapper.java 75 2010-03-16 02:21:41Z MoXie $
 *
...
public final class FieldMapper {
     ...
     * @return
     */
    public static FieldMap<String, Object> toFieldMap(Map<String, Object> map, boolean skipMaped) {
        FieldMap<String, Object> _map = new FieldMap<String, Object>();
        _map.putAll(map);
        String key;
        String[] keys;
        for (Entry<String, Object> entry : map.entrySet()) {
            key = entry.getKey();
            keys = parseKeys(key);
            parseField(_map, keys, entry.getValue());
            if (skipMaped) {
                if (keys.length > 1) {
                    _map.remove(key);
                }
    ...
    private static String[] parseKeys(String str) {
        ...
        for (char ch = it.current(); ch != CharacterIterator.DONE; ch = it.next()) {
            i++;
            if (i == 1) {
                keyList.add(parseKey(strBuffer, it, latestQ));
                ch = it.current();
            }

            if (ch == '[') {
                strBuffer.setLength(0);
                keyList.add(parseKey(strBuffer, it, ch));
            }
     ...
     * @return
     */
    private static String parseKey(StringBuffer strBuffer, CharacterIterator it, char latestQ) {
        String str;
        for (char ch = it.current(); ch != CharacterIterator.DONE; ch = it.next()) {
            if (ch == '[') {
                if (latestQ == 0) {
     ...
     * @return
     */
    private static final boolean isNum(String str) {
        boolean isPass = true;
