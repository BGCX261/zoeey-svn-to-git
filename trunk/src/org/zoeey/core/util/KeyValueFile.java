 ...
 * $Id: KeyValueFile.java 75 2010-03-16 02:21:41Z MoXie $
 *
            ...
            if (isEscape) {
                ...
                if (quoter == '\'') {
                    ...
                    if (ch == 'u') {
                        ...
                        strBuffer.append(StringHelper.toEscapedVal(ch));
                    }
...
//            }
    ...
    public static List<Item> toList(String text) {
        ...
        for (ch = it.current(); ch != CharacterIterator.DONE; ch = it.next()) {
            ...
            val = parseString(it);
            if (ch == '+') {
                val = getLastVal(list, key).concat(val);
            }
     ...
     * @return
     */
    private static String getLastVal(List<Item> list, String name) {
        Item item = null;
        for (Item _item : list) {
            if (name.equals(_item.getKey())) {
                item = _item;
            }
