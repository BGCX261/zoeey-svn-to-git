 ...
 * $Id: FunctionStatement.java 76 2010-03-19 03:21:03Z MoXie $
 * 
...
public class FunctionStatement {
     ...
     * 编译跟踪器
     */
    private final CompileTracker compileTracker;
    /**
     * 语句
     */
    ...
    /**
     * 函数编译器
     * @param compileTracker
     * @param config
     * @param statement
     */
    public FunctionStatement(CompileTracker compileTracker, ZtplConfig config, String statement) {
    ...
    private int seekParams() {
         ...
         * 取得此表的索引位置
         */
        compileTracker.newLocalMap();
        int paramsMapPos = compileTracker.getVarPos();
        /**
         * key1 = $value1 + 8
         */
        int ich = 0;
        ...
        do {
            ...
            if (attrLen > 0) {
                ...
                if (ich == '=') {
                     ...
                     * 仅有属性名 则填充值为 true
                     */
                    compileTracker.localMapPut(paramsMapPos, attrName, compileTracker.newLocal(true));
                } else {
                    // 无属性
                }
            } else {
    ...
    private void execute(int paramsMapPos) {
        ...
        if (funcClass != null) {
            compileTracker.executeFunction(funcClass.getName(), paramsMapPos);
        }
    }
