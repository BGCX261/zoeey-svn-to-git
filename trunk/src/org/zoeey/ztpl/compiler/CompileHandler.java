 ...
 * $Id: CompileHandler.java 76 2010-03-19 03:21:03Z MoXie $
 *
...
import org.zoeey.core.util.BinaryFileHelper;
...
public class CompileHandler {
     ...
     */
    private boolean isNeedCompile(File tplFile, String classFilePath, ZtplConfig config) throws IOException {
    ...
    private String getClassFilePath(File tplFile, String className, File compileDir) {
        ...
        strBuilder.append('/');
        strBuilder.append(CompileHelper.genIdentifier("pack_", tplFile.getParent()));
        strBuilder.append('/');
    ...
    public TemplateAble getCompiledClass(File tplFile, ZtplConfig config) {
        try {
            ...
            String className = CompileHelper.genClassName("Zoeey_tpl_", tplFile);
            ...
            if (isNeedCompile(tplFile, classFilePath, config)) {
                ZtplClassWriter zcw = new ZtplClassWriter(className, ztpl);
                /**
                 * 编译
                 */
                Compiler compiler = new Compiler(config);
                compiler.compile(tplFile, zcw.getCompileTracker());
                /**
                 * 结束编译
                 */
                zcw.endWrite();
                 ...
                 */
                {
                    BinaryFileHelper.write(classFile, zcw.getCode());
                    clazz = classLoader.defineClass(className, zcw.getCode());
                }
            } else {
