 ...
 * $Id: Compiler.java 72 2010-01-23 02:16:24Z MoXie $
 * 
...
class Compiler {
     ...
     * 编译跟踪器
     */
    private CompileTracker compileTracker = null;
    /**
     * 模板配置
     */
     ...
     * 编译器
     * @param tplFile   模板文件
     * @param compileTracker    编译跟踪器
     * @throws Exception
     */
    public void compile(File tplFile, CompileTracker compileTracker) throws Exception {
        this.compileTracker = compileTracker;
        parser(tplFile);
    }
    /**
     * 模板输入流
     ...
     * @throws Exception
     */
    private void parser(File tplFile) throws Exception {
        ...
        do {
            isBoun = isBoundary(currentDelimiter.getLeft());
            {
                if (isBoun) {
                     ...
                     */
                    {
                        compileTracker.write(strBuilder.toString());
                    }
    ...
    private void compileVar(String varExpression) {
        ...
        if (varName.startsWith("!")) {
            compileTracker.writeVarJugeNull(varName.substring(1));
        } else {
            compileTracker.writeVar(varName);
        }
    }
    ...
    private void compileFunction(String functionExpression) {
        FunctionStatement func = new FunctionStatement(compileTracker, config, functionExpression);
